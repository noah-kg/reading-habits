...
def get_book_cover(title, link):
    """
    Function that collects data from the api call. Currently only gets title and thumbnail url.
    title = Title of book from dataframe
    link = Link generated by generate_search_url() function
    ...
    try:
        ...
        if data.get("totalItems", 0) > 0:
            for idx, item in enumerate(data["items"]):
                ...
                # snippet = search_info.get("textSnippet", "N/A")
                # print(f"Title: {title}")
